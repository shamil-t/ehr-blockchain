 ...
 */
function isTaskDefinition(potential) {
 ...
 */
function isPositionalArgumentDefinition(potential) {
    return (typeof potential === "object" &&
        potential !== null &&
        "type" in potential &&
        typeof potential.type === "string" &&
...
export async function validateUserConfig(hooks, config) {
    const validationErrors = collectValidationErrorsForUserConfig(config);
    const results = await hooks.runParallelHandlers("config", "validateUserConfig", [config]);
    return [...validationErrors, ...results.flat(1)];
}
export function collectValidationErrorsForUserConfig(config) {
...
function validatePath(filePath, pathName, index) {
    const validationErrors = [];
    if (typeof filePath !== "string") {
        const messagePrefix = index !== undefined
            ? `paths.${pathName} at index ${index}`
            : `paths.${pathName}`;
        validationErrors.push({
            path: index !== undefined ? ["paths", pathName, index] : ["paths", pathName],
            message: `${messagePrefix} must be a string`,
        });
    }
    return validationErrors;
}
export function validateTasksConfig(tasks, path = [], isPlugin = false) {
    const validationErrors = [];
    for (const [index, task] of tasks.entries()) {
        ...
        switch (task.type) {
            case TaskDefinitionType.EMPTY_TASK: {
                validationErrors.push(...validateEmptyTask(task, [...path, "tasks", index]));
                break;
            }
            case TaskDefinitionType.NEW_TASK: {
                validationErrors.push(...validateNewTask(task, [...path, "tasks", index], isPlugin));
                break;
            }
            case TaskDefinitionType.TASK_OVERRIDE: {
                validationErrors.push(...validateTaskOverride(task, [...path, "tasks", index], isPlugin));
                break;
            }
        }
    }
...
export function validateEmptyTask(task, path) {
    const validationErrors = [];
...
export function validateNewTask(task, path, isPlugin = false) {
    const validationErrors = [];
    if (!Array.isArray(task.id) ||
        !task.id.every((id) => typeof id === "string")) {
        validationErrors.push({
    ...
    if (isObject(task.options)) {
        validationErrors.push(...validateOptions(task.options, [...path, "options"]));
    }
    else {
        validationErrors.push({
            path: [...path, "options"],
    ...
    if (Array.isArray(task.positionalArguments)) {
        validationErrors.push(...validatePositionalArguments(task.positionalArguments, path));
    }
    else {
        validationErrors.push({
...
export function validateTaskOverride(task, path, isPlugin = false) {
    const validationErrors = [];
    if (!Array.isArray(task.id) ||
        !task.id.every((id) => typeof id === "string")) {
        validationErrors.push({
...
function validateActionFields(task, path, isPlugin = false) {
    const validationErrors = [];
    ...
    if (isPlugin && task.inlineAction !== undefined) {
        validationErrors.push({
            ...
            message: "plugins cannot use inline actions. Use a lazy action import instead",
        });
    }
    ...
    if (task.action !== undefined) {
        if (typeof task.action !== "function") {
            validationErrors.push({
                path: [...path, "action"],
                message: "task action must be a lazy import function returning a module with a default export",
            });
        }
    }
    if (task.inlineAction !== undefined) {
        if (typeof task.inlineAction !== "function") {
            validationErrors.push({
                path: [...path, "inlineAction"],
                message: "task inlineAction must be a function implementing the task's behavior",
            });
        }
    }
    return validationErrors;
}
export function validateOptions(options, path) {
    const validationErrors = [];
    for (const [name, option] of Object.entries(options)) {
        ...
        if (option.type !== ArgumentType.STRING_WITHOUT_DEFAULT &&
...
export function validatePositionalArguments(positionalArgs, path) {
    const validationErrors = [];
            ...
            switch (arg.type) {
                ...
                case ArgumentType.FILE: {
                    if (typeof arg.defaultValue !== "string" &&
                        ...
                        validationErrors.push({
                            ...
                            message: "positional argument defaultValue must be a string or an array of strings",
                        });
                    }
                    break;
                }
                case ArgumentType.BOOLEAN: {
                    if (typeof arg.defaultValue !== "boolean" &&
                        ...
                        validationErrors.push({
                            path: [...path, "positionalArguments", index, "defaultValue"],
                            message: "positional argument defaultValue must be a boolean or an array of booleans",
                        });
                    }
                    break;
                }
                ...
                case ArgumentType.FLOAT: {
                    if (typeof arg.defaultValue !== "number" &&
                        ...
                        validationErrors.push({
                            path: [...path, "positionalArguments", index, "defaultValue"],
                            message: "positional argument defaultValue must be a number or an array of numbers",
                        });
                    }
                    break;
                }
                case ArgumentType.BIGINT: {
                    if (typeof arg.defaultValue !== "bigint" &&
                        ...
                        validationErrors.push({
                            path: [...path, "positionalArguments", index, "defaultValue"],
                            message: "positional argument defaultValue must be a bigint or an array of bigints",
                        });
                    }
                    break;
                }
...
export function validatePluginsConfig(plugins, path = []) {
    const validationErrors = [];
    for (const [index, plugin] of plugins.entries()) {
        ...
        if (plugin.dependencies !== undefined) {
            if (typeof plugin.dependencies !== "function") {
                validationErrors.push({
                    ...
                    message: "plugin dependencies must be a function returning an array of functions, each importing a module with a default export",
                });
            }
        }
        if (plugin.hookHandlers !== undefined) {
            if (typeof plugin.hookHandlers === "object" &&
                ...
                for (const [hookName, handler] of Object.entries(plugin.hookHandlers)) {
                    if (typeof handler !== "function") {
                        validationErrors.push({
                            ...
                            message: "plugin hookHandlers must be a lazy import function returning a module with a default export",
                        });
                    }
                }
            }
        ...
        if (plugin.globalOptions !== undefined) {
            if (Array.isArray(plugin.globalOptions)) {
                validationErrors.push(...validateOptions(Object.fromEntries(plugin.globalOptions.entries()), [...path, "plugins", index, "globalOptions"]));
            }
            else {
                validationErrors.push({
        ...
        if (plugin.tasks !== undefined) {
            if (Array.isArray(plugin.tasks)) {
                validationErrors.push(...validateTasksConfig(plugin.tasks, [...path, "plugins", index], true));
            }
            else {
                validationErrors.push({
