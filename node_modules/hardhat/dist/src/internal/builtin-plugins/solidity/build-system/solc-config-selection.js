...
export class SolcConfigSelector {
     ...
     */
    constructor(buildProfileName, buildProfile) {
     ...
     */
    selectBestSolcConfigForSingleRootGraph(subgraph) {
        const roots = subgraph.getRoots();
        assertHardhatInvariant(roots.size === 1, "This method only works for single root graphs");
        ...
        if (overriddenCompiler !== undefined) {
            if (!satisfies(overriddenCompiler.version, versionRange)) {
                return this.#getCompilationJobCreationError(root, subgraph, [overriddenCompiler.version], true);
            }
        ...
        if (matchingVersion === null) {
            return this.#getCompilationJobCreationError(root, subgraph, compilerVersions, false);
        }
        const matchingConfig = this.#buildProfile.compilers.find((x) => x.version === matchingVersion);
        assertHardhatInvariant(matchingConfig !== undefined, `Matching config not found for version '${matchingVersion.toString()}'`);
     ...
     */
    #getCompilationJobCreationError(root, dependencyGraph, compilerVersions, overridden) {
        ...
        if (overridden) {
            ...
            for (const transitiveDependency of this.#getTransitiveDependencies(root, dependencyGraph)) {
                const depOwnRange = transitiveDependency.dependency.content.versionPragmas.join(" ");
            ...
            /* c8 ignore next 5 */
            assertHardhatInvariant(false, "Trying to get the error for an overridden solidity file that has no compatible config, but failed to detect it, as the root and all the dependencies are compatible with the overridden compiler config.");
        }
        ...
        for (const transitiveDependency of this.#getTransitiveDependencies(root, dependencyGraph)) {
            const transitiveDependencyVersionRange = transitiveDependency.versionPragmasPath
     ...
     */
    *#getTransitiveDependencies(root, dependencyGraph, visited = new Set([root])) {
        for (const dependency of dependencyGraph.getDependencies(root)) {
            ...
            for (const transitive of this.#getTransitiveDependencies(file, dependencyGraph, visited)) {
                yield {
...
//# sourceMappingURL=solc-config-selection.js.map