...
import { ImportResolutionErrorType, RootResolutionErrorType, UserRemappingErrorType, } from "../../../../../types/solidity/errors.js";
import { ResolvedFileType } from "../../../../../types/solidity.js";
export function formatProjectRootResolutionError(error) {
    switch (error.type) {
...
export function formatNpmRootResolutionError(error) {
    switch (error.type) {
...
export function formatImportResolutionError(error) {
    switch (error.type) {
...
If you still want to be able to do it, try adding this remapping "${error.suggestedRemapping}" to the "remappings.txt" file in the root of your project.`;
        ...
        case ImportResolutionErrorType.IMPORT_DOES_NOT_EXIST: {
            const packageOrProject = error.resolvedFileType === ResolvedFileType.NPM_PACKAGE_FILE
                ? "the package"
                : "the project";
...
function formatRemappingReference(remapping) {
    return `"${remapping.originalUserRemapping}" from "${shortenPath(remapping.remappingSource)}"`;
}
function formatRemappingErrors(errors) {
    return errors
        .map((error) => `  - "${error.remapping}" from "${shortenPath(error.source)}": ${error.type === UserRemappingErrorType.REMAPPING_WITH_INVALID_SYNTAX ? "Invalid syntax." : "The npm package from its target is not installed."}`)
        .join("\n");
}
function formatResolutionErrorRemappingsOrPackageExportsNotes({ message, subpath, packageExportsResolvedSubpath, userRemapping, isNpmRootResolutionError, }) {
