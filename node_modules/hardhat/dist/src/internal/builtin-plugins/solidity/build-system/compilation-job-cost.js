...
export function estimateCompilationJobCost(job) {
    let totalChars = 0;
    let fileCount = 0;
    ...
    const optimizerRuns = settings.optimizer?.runs ?? SOLC_DEFAULT_OPTIMIZER_RUNS;
    ...
    return ((totalChars + fileOverhead) *
        viaIRMultiplier *
        optimizerMultiplier *
        runsMultiplier);
}
 ...
 */
export function sortCompilationJobsByDescendingCost(compilationJobs) {
    return compilationJobs
        .map((job) => ({ job, cost: estimateCompilationJobCost(job) }))
        .sort((a, b) => b.cost - a.cost)
        .map(({ job }) => job);
}
//# sourceMappingURL=compilation-job-cost.js.map