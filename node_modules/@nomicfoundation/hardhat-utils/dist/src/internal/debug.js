export const NOOP = Object.assign((_format, ..._args) => { }, { enabled: false });
let cached;
/**
 ...
 */
export function parsePatterns(env) {
 ...
 */
function namespaceToRegExp(namespace) {
 ...
 */
export function isEnabled(namespace, env) {
    if (env === "") {
        return false;
    }
    const { include, exclude } = parsePatterns(env);
    return (!exclude.some((p) => p.test(namespace)) &&
...
export function useColors() {
    const isDisabled = process.env.DEBUG_COLORS === "no" || process.env.DEBUG_COLORS === "false";
