 ...
 */
export async function collectAllFilesMatching(dirFrom, results, matches, directoryFilter) {
    const dirContent = await readdirWithFileTypesOrEmpty(dirFrom);
    await Promise.all(dirContent.map(async (dirent) => {
        ...
        if (await isDirectoryDirentAware(absolutePathToFile, dirent)) {
            if (directoryFilter === undefined ||
                ...
                await collectAllFilesMatching(absolutePathToFile, results, matches, directoryFilter);
            }
            return;
        }
 ...
 */
export async function collectAllDirectoriesMatching(dirFrom, results, matches) {
    const dirContent = await readdirWithFileTypesOrEmpty(dirFrom);
    await Promise.all(dirContent.map(async (dirent) => {
        const absolutePathToFile = path.join(dirFrom, dirent.name);
        if (!(await isDirectoryDirentAware(absolutePathToFile, dirent))) {
