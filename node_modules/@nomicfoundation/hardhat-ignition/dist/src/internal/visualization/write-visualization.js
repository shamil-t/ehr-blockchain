...
import { ensureDir, exists, readUtf8File, writeUtf8File, } from "@nomicfoundation/hardhat-utils/fs";
export async function writeVisualization(visualizationPayload, { cacheDir }) {
    const require = createRequire(import.meta.url);
    const templateDir = path.join(require.resolve("@nomicfoundation/ignition-ui/package.json"), "../dist");
    const templateDirExists = await exists(templateDir);
    if (!templateDirExists) {
        throw new HardhatError(HardhatError.ERRORS.IGNITION.INTERNAL.VISUALIZATION_TEMPLATE_DIR_NOT_FOUND, {
            templateDir,
        });
    }
