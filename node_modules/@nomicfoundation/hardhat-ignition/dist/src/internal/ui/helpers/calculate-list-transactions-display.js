...
export async function calculateListTransactionsDisplay(deploymentId, listTransactionsResult, configUrl) {
    ...
    for (const [index, transaction] of listTransactionsResult.entries()) {
        ...
        if (transaction.params !== undefined) {
            text += `  - Params: ${json5.stringify(transaction.params, transactionDisplaySerializeReplacer)}\n`;
        }
...
function transactionTypeToDisplayType(type) {
    switch (type) {
        ...
        default:
            throw new HardhatError(HardhatError.ERRORS.IGNITION.INTERNAL.UNKNOWN_TRANSACTION_TYPE, { type });
    }
}
export function transactionDisplaySerializeReplacer(_key, value) {
