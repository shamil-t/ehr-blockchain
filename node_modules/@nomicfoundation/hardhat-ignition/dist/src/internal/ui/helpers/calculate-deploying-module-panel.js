...
`;
    if (state.isResumed === true) {
        ...
        deployingMessage += `${styleText("bold", `Resuming existing deployment from ${pathFromCwd(state.deploymentDir)}`)}

`;
    }
    deployingMessage += `${styleText("bold", `Deploying [ ${state.moduleName ?? "unknown"} ]${_calculateStrategySuffix(state)}`)}
`;
    if (state.warnings.length > 0) {
        deployingMessage += `\n${styleText("yellow", "Warning - previously executed futures are not in the module:")}\n`;
