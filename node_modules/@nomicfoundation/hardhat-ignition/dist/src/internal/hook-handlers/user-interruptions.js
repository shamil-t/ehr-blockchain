...
export function getUserInterruptionsHandlers() {
    return {
        async displayMessage(context, interruptor, message, next) {
            markPosition();
        ...
        async requestInput(context, interruptor, inputDescription, next) {
            markPosition();
        ...
        async requestSecretInput(context, interruptor, inputDescription, next) {
            markPosition();
            const returnValue = await next(context, interruptor, inputDescription);
            // If we are going to clear the terminal, we wait a small time so it
            // doesn't look like flickering
