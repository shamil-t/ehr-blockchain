...
import { decodeArtifactCustomError, decodeArtifactFunctionCallResult, encodeArtifactDeploymentData, encodeArtifactFunctionCall, executeOnchainInteractionRequest, executeStaticCallRequest, getEventArgumentFromReceipt, getStaticCallExecutionStateResultValue, } from "../internal/execution/execution-strategy-helpers.js";
import { createxArtifact, presignedTx, } from "../internal/execution/strategy/createx-artifact.js";
...
export class Create2Strategy {
    ...
    async init(deploymentLoader, jsonRpcClient) {
        ...
        if (result !== "0x") {
            assertIgnitionInvariant(ethers.keccak256(result) === CREATE_X_DEPLOYED_BYTECODE_HASH, "Deployed CreateX bytecode does not match expected bytecode");
            return;
        }
        const chainId = await this._jsonRpcClient.getChainId();
        // Otherwise if we're not on a local hardhat node, throw an error
        if (chainId !== 31337) {
            throw new HardhatError(HardhatError.ERRORS.IGNITION.STRATEGIES.CREATE_X_NOT_DEPLOYED, { chainId });
        }
    ...
    async *executeDeployment(executionState) {
        assertIgnitionInvariant(this._deploymentLoader !== undefined && this._jsonRpcClient !== undefined, `Strategy ${this.name} not initialized`);
        const artifact = await this._deploymentLoader.loadArtifact(executionState.artifactId);
        const bytecodeToDeploy = encodeArtifactDeploymentData(artifact, executionState.constructorArgs, executionState.libraries);
        const transactionOrResult = yield* executeOnchainInteractionRequest(executionState.id, {
            id: 1,
            type: NetworkInteractionType.ONCHAIN_INTERACTION,
            to: CREATE_X_ADDRESS,
            data: encodeArtifactFunctionCall(createxArtifact, "deployCreate2(bytes32,bytes)", [this.config.salt, bytecodeToDeploy]),
            value: executionState.value,
        }, (returnData) => decodeArtifactFunctionCallResult(createxArtifact, "deployCreate2(bytes32,bytes)", returnData), (returnData) => decodeArtifactCustomError(createxArtifact, returnData));
        if (transactionOrResult.type !==
            OnchainInteractionResponseType.SUCCESSFUL_TRANSACTION) {
            return transactionOrResult;
        }
        const deployedAddress = getEventArgumentFromReceipt(transactionOrResult.transaction.receipt, createxArtifact, CREATE_X_ADDRESS, "ContractCreation", 0, "newContract");
        assertIgnitionInvariant(typeof deployedAddress === "string", "Deployed event should return a string addr property");
        return {
    ...
    async _deployCreateXFactory(client) {
        ...
        while (true) {
            ...
            if (receipt !== undefined) {
                assertIgnitionInvariant(receipt?.contractAddress !== undefined, "CreateX deployment should have an address");
                assertIgnitionInvariant(receipt.contractAddress === CREATE_X_ADDRESS, `CreateX deployment should have the expected address ${CREATE_X_ADDRESS}, instead it is ${receipt.contractAddress}`);
                return;
            }
