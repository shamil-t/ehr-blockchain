...
import { isAccountRuntimeValue, isAddressResolvableFuture, isContractFuture, isEncodeFunctionCallFuture, isFuture, isModuleParameterRuntimeValue, isRuntimeValue, } from "./type-guards.js";
...
export class IgnitionModuleSerializer {
    static serialize(ignitionModule) {
        const argReplacer = (arg) => replaceWithinArg(arg, {
        ...
        switch (future.type) {
            case FutureType.NAMED_ARTIFACT_CONTRACT_DEPLOYMENT:
                const serializedNamedContractDeploymentFuture = {
                    ...
                    type: future.type,
                    dependencies: Array.from(future.dependencies).map((d) => isFuture(d)
...
export class IgnitionModuleDeserializer {
    ...
    static _deserializeArgument(arg, futureLookup) {
        if (this._isSerializedFutureToken(arg)) {
            ...
            if (swappedFuture === undefined) {
                throw new HardhatError(HardhatError.ERRORS.IGNITION.SERIALIZATION.INVALID_FUTURE_ID, {
                    futureId: arg.futureId,
                });
            }
            if (swappedFuture.type === FutureType.CONTRACT_CALL ||
                swappedFuture.type === FutureType.SEND_DATA) {
                throw new HardhatError(HardhatError.ERRORS.IGNITION.SERIALIZATION.INVALID_FUTURE_TYPE, {
    ...
    static _deserializeFuture(serializedFuture, modulesLookup, futuresLookup, contractFuturesLookup, addressResolvableFutureLookup) {
        ...
        switch (serializedFuture.type) {
            case FutureType.NAMED_ARTIFACT_CONTRACT_DEPLOYMENT:
                return new NamedContractDeploymentFutureImplementation(serializedFuture.id, mod, serializedFuture.contractName, serializedFuture.constructorArgs.map((arg) => this._deserializeArgument(arg, futuresLookup)), Object.fromEntries(serializedFuture.libraries.map(([name, lib]) => [
                    name,
            ...
            case FutureType.CONTRACT_DEPLOYMENT:
                return new ArtifactContractDeploymentFutureImplementation(serializedFuture.id, mod, serializedFuture.contractName, serializedFuture.constructorArgs.map((arg) => this._deserializeArgument(arg, futuresLookup)), serializedFuture.artifact, Object.fromEntries(serializedFuture.libraries.map(([name, lib]) => [
                    name,
                    this._lookup(contractFuturesLookup, lib.futureId),
                ])), this._isSerializedFutureToken(serializedFuture.value)
                    ? this._lookup(futuresLookup, serializedFuture.value.futureId)
                    ...
                    : serializedFuture.from);
            case FutureType.NAMED_ARTIFACT_LIBRARY_DEPLOYMENT:
                return new NamedLibraryDeploymentFutureImplementation(serializedFuture.id, mod, serializedFuture.contractName, Object.fromEntries(serializedFuture.libraries.map(([name, lib]) => [
                    name,
                    this._lookup(contractFuturesLookup, lib.futureId),
                    ...
                    : serializedFuture.from);
            case FutureType.LIBRARY_DEPLOYMENT:
                return new ArtifactLibraryDeploymentFutureImplementation(serializedFuture.id, mod, serializedFuture.contractName, serializedFuture.artifact, Object.fromEntries(serializedFuture.libraries.map(([name, lib]) => [
                    name,
                    this._lookup(contractFuturesLookup, lib.futureId),
                ])), this._isSerializedAccountRuntimeValue(serializedFuture.from)
                    ? this._deserializeAccountRuntimeValue(serializedFuture.from)
                    : serializedFuture.from);
            case FutureType.CONTRACT_CALL:
                return new NamedContractCallFutureImplementation(serializedFuture.id, mod, serializedFuture.functionName, this._lookup(contractFuturesLookup, serializedFuture.contract.futureId), serializedFuture.args.map((arg) => this._deserializeArgument(arg, futuresLookup)), this._isSerializedFutureToken(serializedFuture.value)
                    ? this._lookup(futuresLookup, serializedFuture.value.futureId)
                    : this._isSerializedModuleParameterRuntimeValue(serializedFuture.value)
                        ? this._deserializeModuleParameterRuntimeValue(serializedFuture.value) // This is unsafe, but we only serialize valid values
                        : this._deserializedBigint(serializedFuture.value), this._isSerializedAccountRuntimeValue(serializedFuture.from)
                    ...
                    : serializedFuture.from);
            case FutureType.STATIC_CALL:
                return new NamedStaticCallFutureImplementation(serializedFuture.id, mod, serializedFuture.functionName, this._lookup(contractFuturesLookup, serializedFuture.contract.futureId), serializedFuture.args.map((arg) => this._deserializeArgument(arg, futuresLookup)), serializedFuture.nameOrIndex, this._isSerializedAccountRuntimeValue(serializedFuture.from)
                    ? this._deserializeAccountRuntimeValue(serializedFuture.from)
                    : serializedFuture.from);
            case FutureType.ENCODE_FUNCTION_CALL:
                return new NamedEncodeFunctionCallFutureImplementation(serializedFuture.id, mod, serializedFuture.functionName, this._lookup(contractFuturesLookup, serializedFuture.contract.futureId), serializedFuture.args.map((arg) => this._deserializeArgument(arg, futuresLookup)));
            case FutureType.NAMED_ARTIFACT_CONTRACT_AT:
                return new NamedContractAtFutureImplementation(serializedFuture.id, mod, serializedFuture.contractName, this._isSerializedFutureToken(serializedFuture.address)
            ...
            case FutureType.CONTRACT_AT:
                return new ArtifactContractAtFutureImplementation(serializedFuture.id, mod, serializedFuture.contractName, this._isSerializedFutureToken(serializedFuture.address)
                    ? this._lookup(addressResolvableFutureLookup, serializedFuture.address.futureId)
                    : this._isSerializedModuleParameterRuntimeValue(serializedFuture.address)
                        ? this._deserializeModuleParameterRuntimeValue(serializedFuture.address) // This is unsafe, but we only serialize valid values
                        : serializedFuture.address, serializedFuture.artifact);
            case FutureType.READ_EVENT_ARGUMENT:
                return new ReadEventArgumentFutureImplementation(serializedFuture.id, mod, this._lookup(futuresLookup, serializedFuture.futureToReadFrom.futureId), serializedFuture.eventName, serializedFuture.nameOrIndex, this._lookup(contractFuturesLookup, serializedFuture.emitter.futureId), serializedFuture.eventIndex);
            case FutureType.SEND_DATA:
                return new SendDataFutureImplementation(serializedFuture.id, mod, this._isSerializedFutureToken(serializedFuture.to)
    ...
    static _lookup(lookupTable, key) {
        ...
        if (value === undefined) {
            throw new HardhatError(HardhatError.ERRORS.IGNITION.SERIALIZATION.LOOKAHEAD_NOT_FOUND, {
                key,
            });
        }
        return value;
    ...
    static _deserializeModuleParameterRuntimeValue(serialized) {
        ...
        if (serialized.defaultValue !== undefined) {
            ...
            if (typeof parsedDefaultValue === "object" &&
                ...
                defaultValue = new AccountRuntimeValueImplementation(parsedDefaultValue.accountIndex);
            }
            else {
                defaultValue = parsedDefaultValue;
            }
        }
        return new ModuleParameterRuntimeValueImplementation(serialized.moduleId, serialized.name, defaultValue);
    }
}
