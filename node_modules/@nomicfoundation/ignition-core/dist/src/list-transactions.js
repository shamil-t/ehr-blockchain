...
import { ExecutionSateType, } from "./internal/execution/types/execution-state.js";
import { TransactionReceiptStatus, } from "./internal/execution/types/jsonrpc.js";
...
import { TransactionStatus, } from "./types/list-transactions.js";
/**
 ...
 * @public
 */
export async function listTransactions(deploymentDir, _artifactResolver) {
    ...
    if (deploymentState === undefined) {
        throw new HardhatError(HardhatError.ERRORS.IGNITION.LIST_TRANSACTIONS.UNINITIALIZED_DEPLOYMENT, {
            deploymentDir,
        });
    }
    const transactions = [];
    const browserUrl = builtinChains.find(({ chainId }) => chainId === deploymentState.chainId);
    for await (const message of deploymentLoader.readFromJournal()) {
        ...
        assertIgnitionInvariant(doesSendTransactions(exState), "Expected execution state to be a type that sends transactions");
        const networkInteraction = exState.networkInteractions[message.networkInteractionId - 1];
        assertIgnitionInvariant(networkInteraction.type === "ONCHAIN_INTERACTION", "Expected network interaction to be an onchain interaction");
        // this seems redundant, but we use it later to determine pending vs dropped status
        const lastTxIndex = findLastIndex(networkInteraction.transactions, (tx) => tx.hash === message.transaction.hash);
        ...
        switch (exState.type) {
            case ExecutionSateType.DEPLOYMENT_EXECUTION_STATE: {
                transactions.push({
                    ...
                    address: transaction.receipt?.status === TransactionReceiptStatus.SUCCESS
