 ...
 *  - Are not missing.
 */
export function validateLibraryNames(artifact, libraryNames) {
    const errors = [];
    ...
    try {
        ...
        if (requiredLibraries.size !== 0) {
            const fullyQualifiedNames = Array.from(requiredLibraries)
                .map((name) => `* ${name}`)
                .join("\n");
            errors.push(new HardhatError(HardhatError.ERRORS.IGNITION.VALIDATION.MISSING_LIBRARIES, {
                fullyQualifiedNames,
        ...
        assertIgnitionInvariant(e instanceof HardhatError, "Error must be of type HardhatError");
        errors.push(e);
    }
    return errors;
}
 ...
 */
export function linkLibraries(artifact, libraries) {
    ...
    for (const inputName of libraryNames) {
        try {
            ...
            if (sourceName !== undefined && libraryNames.includes(libName)) {
                errors.push(new HardhatError(HardhatError.ERRORS.IGNITION.VALIDATION.CONFLICTING_LIBRARY_NAMES, {
                    inputName,
                    libName,
                    contractName: artifact.contractName,
                }));
...
function parseLibraryName(contractName, libraryName) {
    ...
    if (parts.length > 2) {
        throw new HardhatError(HardhatError.ERRORS.IGNITION.VALIDATION.INVALID_LIBRARY_NAME, {
            libraryName,
            contractName,
        });
    }
...
function getActualNameForArtifactLibrary(artifact, libraryName) {
    ...
    if (sourceName !== undefined) {
        if (artifact.linkReferences[sourceName] === undefined ||
            ...
            throw new HardhatError(HardhatError.ERRORS.IGNITION.VALIDATION.LIBRARY_NOT_NEEDED, {
                libraryName,
                contractName: artifact.contractName,
            });
        }
    ...
    if (bareNameToParsedNames[libName].length > 1) {
        const fullyQualifiedNames = bareNameToParsedNames[libraryName]
            ...
            .join("\n");
        throw new HardhatError(HardhatError.ERRORS.IGNITION.VALIDATION.AMBIGUOUS_LIBRARY_NAME, {
            fullyQualifiedNames,
            libraryName,
            contractName: artifact.contractName,
        });
...
function validateAddresses(artifact, libraries) {
    for (const [libraryName, address] of Object.entries(libraries)) {
        if (address.match(/^0x[0-9a-fA-F]{40}$/) === null) {
            throw new HardhatError(HardhatError.ERRORS.IGNITION.VALIDATION.INVALID_LIBRARY_ADDRESS, {
                address,
                libraryName,
                contractName: artifact.contractName,
            });
