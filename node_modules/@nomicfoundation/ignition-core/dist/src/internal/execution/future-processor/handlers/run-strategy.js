...
import { createExecutionStateCompleteMessage, createExecutionStateCompleteMessageForExecutionsWithOnchainInteractions, } from "../helpers/messages-helpers.js";
 ...
 */
export async function runStrategy(exState, executionStrategy) {
    ...
    if (lastNetworkInteraction === undefined) {
    ...
    else if (lastNetworkInteraction.type === NetworkInteractionType.ONCHAIN_INTERACTION) {
        assertIgnitionInvariant(exState.type !== ExecutionSateType.STATIC_CALL_EXECUTION_STATE, `Unexpected StaticCallExecutionState ${exState.id} with onchain interaction ${lastNetworkInteraction.id} when trying to run a strategy`);
        ...
        assertIgnitionInvariant(confirmedTx !== undefined && confirmedTx.receipt !== undefined, "Trying to advance strategy without confirmed tx in the last network interaction");
        ...
        assertIgnitionInvariant(lastNetworkInteraction.result !== undefined, "Trying to advance strategy without result in the last network interaction");
        response = await strategyGenerator.next(lastNetworkInteraction.result);
    }
    if (response.done !== true) {
        assertIgnitionInvariant(response.value.type !== SIMULATION_SUCCESS_SIGNAL_TYPE, "Invalid SIMULATION_SUCCESS_SIGNAL received");
        return {
