 ...
 */
export async function runStaticCall(client, staticCall) {
 ...
 */
export async function sendTransactionForOnchainInteraction(client, sender, onchainInteraction, nonceManager, decodeSimulationResult, deploymentLoader, futureId) {
    ...
    try {
        ...
        assertIgnitionInvariant(error instanceof Error, "Unexpected error type while resolving failed gas estimation");
        ...
        if (/insufficient funds for transfer/.test(error.message)) {
            throw new HardhatError(HardhatError.ERRORS.IGNITION.EXECUTION.INSUFFICIENT_FUNDS_FOR_TRANSFER, { sender, amount: estimateGasPrams.value.toString() });
        }
        // if the user has insufficient funds to deploy the contract they're trying to deploy
            ...
            throw new HardhatError(HardhatError.ERRORS.IGNITION.EXECUTION.INSUFFICIENT_FUNDS_FOR_DEPLOY, {
                sender,
            });
        }
        // catch-all error for all other errors
        else {
            throw new HardhatError(HardhatError.ERRORS.IGNITION.EXECUTION.GAS_ESTIMATION_FAILED, {
                error: error.message,
            });
        }
    }
...
async function getNextTransactionFees(client, onchainInteraction) {
    ...
    assertIgnitionInvariant("gasPrice" in transactionWithHighestFees.fees, "EIP-1559 transaction was already sent but the currently recommended fees are not EIP-1559");
