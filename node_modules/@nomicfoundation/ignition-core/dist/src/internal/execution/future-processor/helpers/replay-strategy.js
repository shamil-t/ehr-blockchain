...
async function replayExecutionStrategyWithOnchainInteractions(executionState, strategy) {
    assertIgnitionInvariant(executionState.status === ExecutionStatus.STARTED, `Unexpected completed execution state ${executionState.id} when replaying it.`);
    let generator;
 ...
 */
export async function replayStrategy(executionState, strategy) {
    switch (executionState.type) {
        case ExecutionSateType.DEPLOYMENT_EXECUTION_STATE:
            return await replayExecutionStrategyWithOnchainInteractions(executionState, strategy);
        case ExecutionSateType.CALL_EXECUTION_STATE:
            return await replayExecutionStrategyWithOnchainInteractions(executionState, strategy);
        case ExecutionSateType.SEND_DATA_EXECUTION_STATE:
            return await replayExecutionStrategyWithOnchainInteractions(executionState, strategy);
...
function assertValidGeneratorResult(executionStateId, interaction, generatorResult, shouldBeResolved) {
    assertIgnitionInvariant(generatorResult.done !== true, `Unexpected strategy finalization when replaying ${executionStateId}/${interaction.id}`);
    assertIgnitionInvariant(generatorResult.value.type !== SIMULATION_SUCCESS_SIGNAL_TYPE, `Unexpected ${SIMULATION_SUCCESS_SIGNAL_TYPE} when replaying ${executionStateId}/${interaction.id}`);
    assertIgnitionInvariant(interaction.type === generatorResult.value.type, `Unexpected difference between execution strategy request and wat was already executed while replaying ${executionStateId}/${interaction.id}`);
    if (shouldBeResolved === undefined) {
        return;
    }
    if (interaction.type === NetworkInteractionType.STATIC_CALL) {
        assertIgnitionInvariant(interaction.result !== undefined, `Unexpected unresolved StaticCall request when replaying ${executionStateId}/${interaction.id}`);
        return;
    }
    const confirmedTx = interaction.transactions.find((tx) => tx.receipt !== undefined);
    assertIgnitionInvariant(confirmedTx !== undefined, `Unexpected unresolved OnchainInteraction request when replaying ${executionStateId}/${interaction.id}`);
    assertIgnitionInvariant(confirmedTx.receipt !== undefined, `Unexpected unresolved OnchainInteraction request when replaying ${executionStateId}/${interaction.id}`);
}
