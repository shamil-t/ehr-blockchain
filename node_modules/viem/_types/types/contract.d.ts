...
export type ContractFunctionReturnType<abi extends Abi | readonly unknown[] = Abi, mutability extends AbiStateMutability = AbiStateMutability, functionName extends ContractFunctionName<abi, mutability> = ContractFunctionName<abi, mutability>, args extends ContractFunctionArgs<abi, mutability, functionName> = ContractFunctionArgs<abi, mutability, functionName>> = abi extends Abi ? Abi extends abi ? unknown : AbiParametersToPrimitiveTypes<ExtractAbiFunctionForArgs<abi, mutability, functionName, args>['outputs'], 'outputs', true> extends infer types ? types extends readonly [] ? void : types extends readonly [infer type] ? type : types : never : unknown;
...
export type AbiItemArgs<abi extends Abi | readonly unknown[] = Abi, name extends AbiItemName<abi> = AbiItemName<abi>> = AbiParametersToPrimitiveTypes<ExtractAbiItem<abi extends Abi ? abi : Abi, name>['inputs'], 'inputs', true> extends infer args ? [args] extends [never] ? readonly unknown[] : args : readonly unknown[];
export type ExtractAbiItemForArgs<abi extends Abi, name extends AbiItemName<abi>, args extends AbiItemArgs<abi, name>> = ExtractAbiItem<abi, name> extends infer abiItem extends AbiItem & {
    ...
    [k in keyof abiItems]: (readonly [] extends args ? readonly [] : args) extends AbiParametersToPrimitiveTypes<abiItems[k]['inputs'], 'inputs', true> ? abiItems[k] : never;
 ...
 * @internal
 */
export type LogTopicType<primitiveType = Hex, topic extends LogTopic = LogTopic> = topic extends Hex ? primitiveType : topic extends readonly Hex[] ? readonly primitiveType[] : topic extends null ? null : never;
/**
 * @internal
 */
